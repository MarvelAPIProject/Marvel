...
async function fetchItems(contentType, searchTerm = '', page = 1, universeFilter = 'all') {
  try {
    ...
    // Pedir más items para compensar los que no tienen imagen
    const fetchLimit = charactersPerPage * 3;
    
    const ts = Math.floor(Date.now() / 1000);
    const hash = await generateHash(ts);
    
    // Determinar el endpoint según el tipo de contenido
    let endpoint = '';
    switch (contentType) {
      case 'characters':
        endpoint = 'characters';
    ...
    if (searchTerm) {
      if (contentType === 'characters') {
        ...
        params.append('titleStartsWith', searchTerm);
      }
    }
    ...
    try {
      ...
      // Añadir información de universo a los items
      let items = data.data.results.map(item => {
      ...
      if (universeFilter !== 'all' && universeMapping[universeFilter]) {
        items = items.filter(item => 
          item.universe === universeMapping[universeFilter]
        );
      }
      ...
      if (universeFilter !== 'all' || validItems.length < items.length) {
        const ratio = items.length > 0 ? validItems.length / items.length : 0;
        totalCharacters = Math.max(Math.ceil(totalCharacters * ratio), validItems.length);
      }
      
      // Asegurar que devolvemos el número correcto de items por página
      return validItems.slice(0, charactersPerPage);
    ...
    displayError("Error al generar la autenticación. Por favor, intenta de nuevo.");
    return [];
  }
}

// Obtener favoritos del usuario (usa la función de favorites.js)
async function fetchFavorites() {
  // Se implementa completamente en favorites.js
  return []; // Esta función solo existe como placeholder, el contenido real vendrá de fetchAndDisplayFavorites
}
...
function displayCharacters(characters) {
  ...
  characters.forEach(character => {
    ...
    if (favoriteIcon) {
      if (isInFavorites && isInFavorites(character.id, 'characters')) {
...
function displayComics(comics) {
  ...
  comics.forEach(comic => {
    ...
    // Configure favorite icon if it exists
    if (favoriteIcon) {
      if (isInFavorites && isInFavorites(comic.id, 'comics')) {
        favoriteIcon.classList.add('active');
        favoriteIcon.querySelector('i').classList.remove('far');
        favoriteIcon.querySelector('i').classList.add('fas');
      } else {
...
document.addEventListener('DOMContentLoaded', function() {
  // Make sure favorites.js runs first
